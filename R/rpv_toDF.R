...
rpv_toDF <- function(
  ...
  if (inherits(rpv_results, "eiCompare")) {
    ...
    fun <- function(x) {
      ...
      # Strip name tag suffix (e.g. "_Iter", "_RxC") from column names
      tag <- rpv_results$name
      if (!is.null(tag) && tag != "") {
        colnames(df) <- sub(paste0("_", tag, "$"), "", colnames(df))
      }
      df <- df %>% dplyr::select(-dplyr::any_of("sd"))
  ...
  newcols <- gsub("ci(_95)?_lower", "Lower_Bound", newcols)
  newcols <- gsub("ci(_95)?_upper", "Upper_Bound", newcols)
