plot.ei <- function(x, ...) {
  ...
  .repar <- function(Bb0, Bw0, sb0, sw0, rho0, Bb0v, Bw0v, Zb, Zw) {
    sb <- exp(sb0)
    sw <- exp(sw0)
    bb <- Bb0 * (0.25 + sb^2) + 0.5 + as.matrix(apply(Zb, 2, function(x) {
      x -
        mean(x)
    })) %*% as.matrix(Bb0v)
    bw <- Bw0 * (0.25 + sw^2) + 0.5 + as.matrix(apply(Zw, 2, function(x) {
      x -
        mean(x)
    })) %*% as.matrix(Bw0v)
  ...
  .tomogl <- function(ei.object, lci = TRUE) {
    ...
    vars <- .repar(
      Bb0, Bw0, sb0, sw0, rho0, Bb0v, Bw0v, Zb,
      Zw
    )
    bb <- vars[1:length(x)]
  ...
  .bndplot <- function(dbuf) {
    ...
    bnds <- bounds(form,
      data = data, rows = rows, column = cols,
      threshold = 0
    )
    ...
    for (i in 1:dim(hstr)[1]) {
      if ((exp1[i] + exp2[i] + exp3[i] + exp4[i]) == 0) {
        xaxs <- c(hstr[i, 1], lstr[i, 1], lend[i, 1], hend[
          i,
          1
        ])
        yaxs <- c(hstr[i, 2], lstr[i, 2], lend[i, 2], hend[
          i,
          2
        ])
        side1 <- c(xaxs, xaxs[1])
      ...
      if ((exp1[i] + exp2[i]) == 0 & (exp1[i] + exp2[i] + exp3[i] +
        exp4[i]) != 0) {
        xaxs <- c(hstr[i, 1], lstr[i, 1], kink2x, lend[
          i,
          1
        ], hend[i, 1])
        xaxs <- ifelse(is.nan(xaxs), 0, xaxs)
        yaxs <- c(hstr[i, 2], lstr[i, 2], kink2y, lend[
          i,
          2
        ], hend[i, 2])
        yaxs <- ifelse(is.nan(yaxs), 0, yaxs)
  ...
  .movie <- function(ei.object) {
    ...
    while (input != "s") {
      input <- .postonce(
        input, last.input, betab, betaw, betabs,
        betaws
      )
      last.input <- input
      input <- .getinput()
