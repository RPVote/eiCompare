...
#'
#'
#' @param object \code{object} An ei object from the function ei.
#' @param \dots \code{\dots{}} A list of options to return in graphs. See
#' values below.
#' @note Used within ei_est_gen() function
...
summary.ei <- function(object, ...) {
  ...
  if ("psi" %in% names(object)) {
    ...
    output <- list(
      ei1$erho, ei1$esigma, ei1$ebeta, n, ei1$resamp,
  ...
  if (!("psi" %in% names(object))) {
    ...
    colnames(ab) <- c("betab", "betaw")
    output <- list(
      ei1$erho, ei1$esigma, ei1$ebeta, n, mle,
      ab, ei1$precision
    )
    names(output) <- c(
      "Erho", "Esigma", "Ebeta", "N", "Maximum likelihood results in scale of estimation (and se's)",
