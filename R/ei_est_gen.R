          ...
          sample = 1000, tomog = F, density_plot = F, beta_yes = F, ...) 
{
  ...
  for (k in 1:length(race_group)) {
    ...
    for (i in 1:length(cand_vector)) {
      ...
      try(ei_out <- ei::ei(form, total = total, erho = rho, 
                           data = data, sample = sample, ...), silent = T)
      gm <- geterrmessage()
      ...
      beta_stan_err <- eiread(ei_out, "betab", "sbetab", 
                              "betaw", "sbetaw")
      ...
      #  Weighted Mean #
      eimean <- data.frame(c(min_b, min_ste), c(non_b, 
                                                non_ste))
  ...
  if (length(race_group) == 1) {
    ...
    race_group_table <- data.frame(lapply(race_group_table, 
                                          list_extract))
    race_group_table <- race_group_table[, c(1, seq(2, ncol(race_group_table), 
                                                    2))]
  }
  tot <- colSums(race_group_table[seq(1, nrow(race_group_table), 
                                      2), 2:ncol(race_group_table)])
