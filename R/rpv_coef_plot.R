...
rpv_coef_plot <- function(
  ...
  if (is.null(rpvDF)) {
    stop("you must include rpvDF argument")
  }

  # make sure necessary columns are included
  dif <- setdiff(
    c("party", "voter_race", "estimate", "lower_bound", "upper_bound"),
    colnames(rpvDF)
  )

  if (length(dif) > 0) {
    stop(paste(
      "rpvDF is missing the following fields:",
      paste(dif, collapse = ", ")
    ))
  }
  ...
  if (is.null(race_order)) {
    race_order <- sort(unique(rpvDF$voter_race))
  }
  # set factor
  ...
  if (is.null(colors)) {
    if (len_race == 2) {
      ...
      ggplot_color_obj <- viridis::scale_color_viridis(
        drop = FALSE,
        discrete = TRUE,
        option = "turbo",
        alpha = .8
