...
With that, let's run ei_ter to output the first set of visuals. If you want to to opt to create plots for `ei_iter` you'll need to set `plots` to `TRUE`. In addition, if you'd like to specificy where you would like to save your plots, you can specify a `plots_path`. In this example we'll just set it to where this vignette lives.
