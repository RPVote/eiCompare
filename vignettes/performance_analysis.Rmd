...
```{r cvap_map, fig.height=6, fig.width=7.2, message=FALSE, warning=FALSE}
# Load the map
data("ersd_maps")
...
```{r plot_voter_w_ward,  fig.height=6, fig.width=7.2, message=FALSE, warning=FALSE}
...
These race probabilities have to be merged back into the original voter file, producing a final voter file.
...
To see this function in action, we'll use it on the East Ramapo voter file. Ordinarily, this voter file would have the same components as the toy voter file, at a minimum (voter id, surname, location). For privacy reasons, we've replaced the voter IDs with new one and have already performed the spatial joins between 

The performance analysis function can handle all these edge cases: it will spatial join with district and Census (or it won't), depending on the user's needs. 

```{r performance_analysis}
load("ramapo2018.rda")
```

```{r performance_analysis2}
results <- eiCompare::performance_analysis(
  voter_file = ramapo2018,
  district_shape = ersd_maps,
  census_data = rockland_census,
...
```{r visualize_results, fig.height=5, fig.width=5, message=FALSE, warning=FALSE}
