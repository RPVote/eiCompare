...
`eiCompare` enables users to analyze and compare three different approaches to EI, The three approaches are Goodman's EI, Iterative EI, and RxC EI. For more information about these techniques, see the [technical details](technical.details.com) page of our website.

We can run these methods easily with their respective functions. `ei_rxc()` and `ei_iter` may take some time to run, since they both run simulations to compute their estimates. 
...
summary.eic(ei_results_rxc)
```

```{r}
# results <- list(ei_results_iter, ei_results_rxc)
#
# for (i in 1:length(list)) {
#
# }
#
# ei_results_iter$estimates
# ei_results_rxc$estimates
```
